...
import api from '../api';
import styles from './Deadstock.module.css';

const fetchDeadstock = async (page) => (await api.get(`deadstock/?page=${page}`)).data;

const Deadstock = () => {
  const [page, setPage] = React.useState(1);
  const { data: response, isLoading, isFetching, isError } = useQuery({
    queryKey: ['deadstockReport', page],
    queryFn: () => fetchDeadstock(page)
  });
  ...
  if (isError) {
    return <div className={styles.loading}>Error loading deadstock report.</div>;
  }

  return (
    <div className={styles.container}>
      <div className={styles.header}>
        <h1 className={styles.title}>Deadstock Analysis</h1>
        <p className={styles.subtitle}>Items that have been purchased but have zero sales.</p>
      </div>
          ...
          <tbody>
            {report.map((item, idx) => (
              <tr key={idx}>
                ...
                <td>
                  <span className={styles.dangerBadge}>Deadstock</span>
                </td>
              </tr>
            ))}
            {report.length === 0 && (
              <tr>
                <td colSpan="5" style={{ textAlign: 'center', padding: '24px' }}>No deadstock items found</td>
              </tr>
            )}
          </tbody>
        </table>
